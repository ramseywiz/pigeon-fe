the idea here is to be able to login with a CCS account, and then upload event details for an event. the current idea is to only allow account creation with cougarcs domains.
...
to run the edge function locally, do:
npx supabase login
npx supabase functions deploy restrict-domain --no-verify-jwt
